#!/bin/bash
# =============================================================================
# EMPCL ERP — Server SSH Key & Root Password Setup Script
# File: setup-ssh.sh (Project Root)
# =============================================================================

set -e

SSH_DIR="$HOME/.ssh"
AUTH_KEYS="$SSH_DIR/authorized_keys"
KEY="ssh-rsa  lenovo@DESKTOP-J0SQ8M0"

echo "=========================================="
echo "    ERP Server Security Setup Script      "
echo "=========================================="
echo ""

# 1. Setup SSH Key
echo ">>> Setting up SSH Key..."
mkdir -p "$SSH_DIR"
chmod 700 "$SSH_DIR"

if [ -f "$AUTH_KEYS" ] && grep -Fq "lenovo@DESKTOP-J0SQ8M0" "$AUTH_KEYS"; then
    echo ">>> SSH key is already registered in authorized_keys!"
else
    echo ">>> Appending SSH key to authorized_keys..."
    echo "$KEY" >> "$AUTH_KEYS"
fi
chmod 600 "$AUTH_KEYS"
echo ""

# 2. Reset Root Password
echo ">>> Do you want to set a new root password? (y/n)"
read -r change_pass
if [[ "$change_pass" =~ ^([yY][eE][sS]|[yY])$ ]]; then
    echo ">>> Running passwd command to update root password..."
    passwd root
else
    echo ">>> Skipping password update."
fi

echo ""
echo "=========================================="
echo ">>> Setup Complete!"
echo ">>> You can now use SSH/SCP without a password from your local machine."
echo "=========================================="
