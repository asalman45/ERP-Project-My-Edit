...
async function seedData() {
  console.log(`Starting to seed physical inventory balances for ${rawData.length} Raw Materials...`);

  // Try to find main RM warehouse
  let mainLocation = await prisma.location.findFirst({
    where: { name: { contains: 'Raw Material' } }
  });

  if (!mainLocation) {
    mainLocation = await prisma.location.findFirst();
  }

  if (!mainLocation) {
    console.error("No locations found in the database. Cannot insert inventory records.");
    process.exit(1);
  }

  console.log(`Using Location: ${mainLocation.name} (${mainLocation.location_id})`);

  let count = 0;
  for (const item of rawData) {
    // Get the material by material_code to get the material_id
    const material = await prisma.material.findUnique({
      where: { material_code: item.code }
    });

    if (!material) {
      console.warn(`Material code ${item.code} not found. Skipping...`);
      continue;
    }

    // Setting a base quantity for demo purposes
    // Using 500 for most random things, but a bit more for pieces
    const openingStockQty = item.uom.toUpperCase() === 'PCS' ? 500 : 100;

    // Check if inventory already exists for this material at this location
    let inventoryRecord = await prisma.inventory.findFirst({
      where: { 
        material_id: material.material_id,
        location_id: mainLocation.location_id
      }
    });

    if (inventoryRecord) {
      // Setup the inventory update logic
      inventoryRecord = await prisma.inventory.update({
        where: { inventory_id: inventoryRecord.inventory_id },
        data: {
          quantity: inventoryRecord.quantity + openingStockQty
        }
      });
      console.log(`Updated inventory for ${item.code} to ${inventoryRecord.quantity}`);
    } else {
      // Create new inventory logic
      ...
      console.log(`Created new inventory for ${item.code} with quantity ${openingStockQty}`);
    }

    // Insert an initial entry transaction to back the balance
    await prisma.inventoryTxn.create({
      data: {
        inventory_id: inventoryRecord.inventory_id,
        material_id: material.material_id,
        txn_type: 'ADJUSTMENT', // Initial load
        quantity: openingStockQty,
        location_id: mainLocation.location_id,
        reference: 'Initial Stock Dump 2026',
  ...
  console.log(`Successfully seeded inventory balances for ${count} raw materials.`);
}
